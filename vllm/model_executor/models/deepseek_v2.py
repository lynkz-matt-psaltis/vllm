...
# Copyright 2022 DeepSeek-AI and The HuggingFace Inc. team. All rights reserved.
#
...
"""Inference-only Deepseek2 model."""
...
class Deepseek2MoE(nn.Module):
    """A tensor-parallel MoE implementation for Deepseek2 that shards each expert
    across all ranks.
    ...
    def __init__(
        ...
        if self.use_fp8:
            ...
            self.w13_scale = nn.Parameter(
                torch.ones(self.num_total_experts, 2, dtype=torch.float32),
                requires_grad=False,
            )
            ...
            for expert_id in range(self.num_total_experts):
                ...
                for shard_id in range(2):
                    dq_weight = per_tensor_dequantize(
                        self.w13_weight[expert_id][start : start + shard_size, :],
...
class Deepseek2DecoderLayer(nn.Module):
    ...
    def __init__(
        ...
        self.self_attn = Deepseek2Attention(
        ...
        self.block_sparse_moe = Deepseek2MoE(
...
class Deepseek2Model(nn.Module):
    ...
    def __init__(
        ...
        self.layers = nn.ModuleList(
            [
                Deepseek2DecoderLayer(config, cache_config, quant_config=quant_config)
...
class Deepseek2ForCausalLM(nn.Module):
